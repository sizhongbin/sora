...
const db = new nedb({ filename: middleware.dbPath, autoload: true });
...
async function signUp(mail, pass) {
  return new Promise(async resolve => {
    try {
      ...
      const data = {
        table: 'account',
        createTime: middleware.now(),
        updateTime: middleware.now(),
        isDeleted: 0,
        mail: mail,
        pass: pass
      }
  ...
  return new Promise(async resolve => {
    try {
      ...
      db.find(query, (err, doc) => {
        if (err) throw err;
        if (doc.length === 0)
          return resolve({ error: 'Incorrect mail' });
        else if (doc[0].pass !== pass)
          return resolve({ error: 'Incorrect pass' });
        else
          return resolve({ data: doc[0] });
      });
    } catch (err) {
      return resolve({ error: err });
    }
...
router.post('/signup', async function (req, res) {
  ...
  if (!req.body.mail || !req.body.pass) {
    console.log('X [400] Params not acceptable.');
    console.log('### Account API - Sign Up End ###');
    res.status(400).send('Empty param(s)');
    return;
  };
  console.log('O Params accepted.');
...
router.post('/signin', async function (req, res) {
  ...
  if (results.error === 'Incorrect mail' || results.error === 'Incorrect pass') {
    console.log(`X [400] ${results.error}.`);
    console.log('### Account API - Sign In End ###');
    res.status(400).send(results.error);
    return;
